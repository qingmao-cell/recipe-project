    ...
    try {
      ...
      if (data.success) {
        if (data.warning === "content-partial") {
          router.push(`/${locale}/recipes/${data.recipe.id}/edit`);
        } else {
          router.push(`/${locale}/recipes`);
        }
      } else {
        throw new Error(data.error || t('collectError'));
      }
    } catch (e) {
      const msg = e instanceof Error ? e.message : t('collectError');
      setError(msg);
    } finally {
      setLoading(false);
    }
  ...
  const handleImageUpload = async (file: File) => {
    ...
    try {
      ...
      if (data.data) {
        ...
        // 确保locale有效，如果没有则使用默认值
        const targetLocale = locale || 'zh';
        router.push(`/${targetLocale}/recipes/create?from=ocr`);
      } else {
        throw new Error(data.error || t('ocrError'));
      }
    } catch (e) {
      const msg = e instanceof Error ? e.message : t('ocrError');
      setError(msg);
    } finally {
      setLoading(false);
      // 清空文件输入
    ...
    >
      ...
      <div className="absolute top-6 right-6 z-20 flex gap-6">
        ...
        {ENABLE_LABS && (
          <Link
            href={`/${locale}/lab`}
            className="group inline-flex items-center gap-2 text-base font-medium text-[#ff8c42] hover:text-[#ff7a2e] transition-colors opacity-70"
          >
            <span className="transition-transform group-hover:-translate-x-0.5">
              🧪
            </span>
      ...
      <div className="relative z-10 flex items-start justify-center min-h-screen px-4 pt-32">
        <div className="w-full max-w-2xl">
          <div className="flex items-center gap-3">
            ...
            >
              {loading ? t('collecting') : t('collect')}
            </button>
          </div>
